...
import { buildTracks, experiences, projects, skillGroups, type ProjectCategory } from '@/data/portfolio';
import {
  Route,
  Switch,
  useLocation,
...
function Home() {
  ...
  const downloadCv = () => showNotice('CV PDF placeholder — add the final PDF at /cv/amine-boulila.pdf.');
      ...
      <main id="top">
        <section className="relative isolate overflow-hidden border-b border-border/70">
          ...
          <div className="mx-auto grid max-w-7xl gap-14 px-5 pb-24 pt-16 md:grid-cols-[1.1fr_.9fr] md:items-end md:pb-32 md:pt-24 lg:px-10">
            <div className="reveal">
              ...
              <p className="mt-8 max-w-xl text-lg leading-8 text-muted-foreground md:text-xl">I’m <strong className="font-semibold text-foreground">Amine Boulila</strong> — a software engineering student building AI-powered applications, backend systems and cloud-ready software.</p>
              ...
              <div className="mt-8 flex flex-wrap items-center gap-5 font-mono text-xs text-muted-foreground">
                <button onClick={() => placeholderLink('GitHub')} className="inline-flex items-center gap-2 transition hover:text-primary" data-testid="button-hero-github"><Github size={15} /> GitHub</button>
                <button onClick={() => placeholderLink('LinkedIn')} className="inline-flex items-center gap-2 transition hover:text-primary" data-testid="button-hero-linkedin"><Linkedin size={15} /> LinkedIn</button>
                <a href="#contact" className="inline-flex items-center gap-2 transition hover:text-primary" data-testid="link-hero-email"><Mail size={15} /> Email</a>
              </div>
            </div>
            <div className="reveal reveal-delay-2 relative min-h-[22rem] md:min-h-[28rem]">
              <div className="absolute inset-0 rounded-xl border border-border/80 bg-card/55 p-4 backdrop-blur-sm">
                ...
                <div className="relative mt-5 h-[calc(100%-2.4rem)] overflow-hidden rounded-lg bg-background/70">
                  ...
                  <div className="absolute bottom-5 left-5 right-5 border-l border-primary pl-3 font-mono text-[10px] leading-5 text-muted-foreground"><span className="text-primary">const</span> direction = [<br /><span className="pl-3 text-foreground">'ai'</span>, <span className="text-foreground">'backend'</span>, <span className="text-foreground">'cloud'</span><br />];<span className="cursor-blink ml-1 text-primary">_</span></div>
              ...
              <div className="absolute -bottom-5 -left-3 rounded-lg border border-border bg-card px-4 py-3 font-mono text-[10px] uppercase tracking-[.1em] shadow-xl md:-left-8"><span className="mr-2 inline-block size-1.5 rounded-full bg-primary" /> Open to PFE · Jan 2027</div>
            </div>
          </div>
        ...
        <section id="about" className="mx-auto max-w-7xl scroll-mt-20 px-5 py-24 lg:px-10 lg:py-32">
          ...
          <div className="mt-14 grid gap-12 md:grid-cols-[.72fr_1.28fr] md:gap-24">
            ...
            <div className="max-w-2xl space-y-6 text-base leading-8 text-muted-foreground"><p>I’m studying Software Engineering at the Faculty of Sciences of Tunis, with a focus on the systems behind useful products — the APIs, data flows, models and infrastructure that make an idea dependable.</p><p>My work sits across AI/ML, backend engineering, cloud and DevOps, data engineering, and full-stack development. I like solving practical engineering problems and turning experiments into software that can be understood, extended and run.</p><p className="text-foreground">I’m looking for a challenging <span className="text-primary">4–6 month PFE opportunity starting January 2027</span>, with the possibility of continuing full-time afterward.</p></div>
          </div>
          <div className="mt-16 grid gap-px overflow-hidden rounded-xl border border-border bg-border sm:grid-cols-3">
            <InfoCell icon={<Layers3 size={18} />} label="Working across" value="AI · backend · cloud" />
            <InfoCell icon={<Code2 size={18} />} label="Approach" value="Prototype → production" />
            <InfoCell icon={<ShieldCheck size={18} />} label="Currently" value="Open to PFE · 01.2027" />
          </div>
        </section>
        ...
        <section id="github" className="mx-auto max-w-7xl scroll-mt-20 px-5 py-24 lg:px-10 lg:py-32"><div className="grid gap-12 md:grid-cols-[.8fr_1.2fr] md:items-end"><div><p className="font-mono text-[11px] uppercase tracking-[.18em] text-primary">07 / Building in public</p><h2 className="mt-5 max-w-lg text-4xl font-bold tracking-[-.04em] sm:text-5xl">The code is part of the conversation.</h2></div><div className="max-w-xl"><p className="text-lg leading-8 text-muted-foreground">I like turning what I learn into working software, experiments and reusable tools.</p><button onClick={() => placeholderLink('GitHub')} className="mt-6 inline-flex items-center gap-2 rounded-md border border-border px-4 py-3 font-mono text-xs uppercase tracking-[.1em] transition hover:border-primary hover:text-primary" data-testid="button-github-public"><Github size={15} /> Visit GitHub <ExternalLink size={14} /></button></div></div><div className="mt-14 rounded-xl border border-dashed border-border bg-card/55 p-6 md:p-8"><div className="flex flex-col justify-between gap-4 border-b border-border pb-5 sm:flex-row sm:items-center"><div><p className="font-mono text-xs text-primary">[GitHub username]</p><p className="mt-1 text-sm text-muted-foreground">Repository cards will connect here when the public profile URL is ready.</p></div><GitBranch className="text-muted-foreground" size={20} /></div><div className="grid gap-3 pt-5 md:grid-cols-3"><RepoPlaceholder title="Latest repository" /><RepoPlaceholder title="Experiment / lab" /><RepoPlaceholder title="Developer tool" /></div></div></section>

        <section id="contact" className="scroll-mt-20 border-t border-border"><div className="mx-auto max-w-7xl px-5 py-24 lg:px-10 lg:py-32"><div className="grid gap-14 lg:grid-cols-[1fr_.85fr] lg:gap-24"><div><p className="font-mono text-[11px] uppercase tracking-[.18em] text-primary">08 / Contact</p><h2 className="mt-5 max-w-xl text-5xl font-bold leading-[.98] tracking-[-.055em] sm:text-6xl">Let’s build something useful.</h2><p className="mt-7 max-w-lg text-lg leading-8 text-muted-foreground">I’m currently looking for a PFE opportunity starting January 2027 in AI/ML, backend engineering, cloud/DevOps or related software engineering roles.</p><div className="mt-10 flex flex-wrap gap-3"><button onClick={() => placeholderLink('LinkedIn')} className="inline-flex items-center gap-2 rounded-md border border-border px-4 py-3 font-mono text-xs transition hover:border-primary hover:text-primary" data-testid="button-contact-linkedin"><Linkedin size={15} /> LinkedIn</button><button onClick={() => placeholderLink('GitHub')} className="inline-flex items-center gap-2 rounded-md border border-border px-4 py-3 font-mono text-xs transition hover:border-primary hover:text-primary" data-testid="button-contact-github"><Github size={15} /> GitHub</button><button onClick={downloadCv} className="inline-flex items-center gap-2 rounded-md border border-border px-4 py-3 font-mono text-xs transition hover:border-primary hover:text-primary" data-testid="button-contact-cv"><Download size={15} /> Download CV</button></div></div><form onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; const data = new FormData(form); const subject = encodeURIComponent(`Portfolio message from ${data.get('name') || 'a visitor'}`); const body = encodeURIComponent(`Name: ${data.get('name')}\nEmail: ${data.get('email')}\n\n${data.get('message')}`); showNotice('Opening your email client with a mailto draft.'); window.location.href = `mailto:?subject=${subject}&body=${body}`; }} className="rounded-xl border border-border bg-card p-6 md:p-8"><div className="mb-8 flex items-center justify-between"><span className="font-mono text-xs text-primary">// send a note</span><Send size={17} className="text-muted-foreground" /></div><div className="grid gap-5"><label className="grid gap-2"><span className="font-mono text-[10px] uppercase tracking-[.12em] text-muted-foreground">Name</span><input required name="name" type="text" placeholder="Your name" className="rounded-md border border-input bg-background px-3.5 py-3 text-sm outline-none transition placeholder:text-muted-foreground/60 focus:border-primary focus:ring-2 focus:ring-primary/15" data-testid="input-contact-name" /></label><label className="grid gap-2"><span className="font-mono text-[10px] uppercase tracking-[.12em] text-muted-foreground">Email</span><input required name="email" type="email" placeholder="you@company.com" className="rounded-md border border-input bg-background px-3.5 py-3 text-sm outline-none transition placeholder:text-muted-foreground/60 focus:border-primary focus:ring-2 focus:ring-primary/15" data-testid="input-contact-email" /></label><label className="grid gap-2"><span className="font-mono text-[10px] uppercase tracking-[.12em] text-muted-foreground">Message</span><textarea required name="message" rows={5} placeholder="What are you building?" className="resize-none rounded-md border border-input bg-background px-3.5 py-3 text-sm outline-none transition placeholder:text-muted-foreground/60 focus:border-primary focus:ring-2 focus:ring-primary/15" data-testid="input-contact-message" /></label><button type="submit" className="inline-flex items-center justify-center gap-2 rounded-md bg-primary px-4 py-3.5 text-sm font-semibold text-primary-foreground transition hover:-translate-y-0.5 hover:shadow-[0_8px_22px_hsl(var(--primary)/.18)]" data-testid="button-contact-submit">Compose email <ArrowUpRight size={16} /></button><p className="font-mono text-[10px] leading-5 text-muted-foreground">No backend configured. This form uses your email client via mailto fallback.</p></div></form></div></div></section>
      </main>
