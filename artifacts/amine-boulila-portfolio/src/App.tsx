...
function ProjectCard({ project, index, onOpen }: { project: (typeof projects)[number]; index: number; onOpen: () => void }) {
  const openOnKey = (event: React.KeyboardEvent<HTMLElement>) => {
   ...
   return <article role="button" tabIndex={0} onClick={onOpen} onKeyDown={openOnKey} className={`group relative cursor-pointer overflow-hidden rounded-xl border bg-card transition duration-300 hover:-translate-y-1 hover:border-primary/50 hover:shadow-[0_18px_50px_hsl(var(--background)/.25)] ${project.standout ? 'border-primary/30 lg:col-span-2' : 'border-border'}`} data-testid={`card-project-${project.id}`} aria-label={`Open details for ${project.name}`}>
     ...
     <div className="flex min-h-64 flex-col justify-between p-6 md:min-h-72 md:p-8"><div><div className="mb-12 flex items-center gap-3"><span className="grid size-10 place-items-center rounded-md bg-primary/10 text-primary">{project.category === 'AI / ML' ? <Sparkles size={18} /> : project.category === 'Full-stack' ? <Layers3 size={18} /> : <Server size={18} />}</span><span className="font-mono text-[10px] uppercase tracking-[.15em] text-primary">{project.eyebrow}</span></div><h3 className="max-w-3xl text-2xl font-semibold tracking-[-.03em] md:text-3xl">{project.name}</h3><p className="mt-4 max-w-2xl leading-7 text-muted-foreground">{project.description}</p></div><div className="mt-10 flex items-center gap-2 border-t border-border pt-5 font-mono text-[10px] uppercase tracking-[.12em] text-primary">View project details <ArrowUpRight size={13} className="transition group-hover:-translate-y-0.5 group-hover:translate-x-0.5" /></div></div>
  </article>;
}
