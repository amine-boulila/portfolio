...
function Home() {
  ...
  const [lightMode, setLightMode] = useState(
    () => localStorage.getItem("amine-theme") === "light",
  );
  ...
  const featuredProjects = useMemo(
    () => orderProjects(filteredProjects, featuredProjectIds),
    [filteredProjects],
  );
  const moreWorkProjects = useMemo(
    () => orderProjects(filteredProjects, moreWorkProjectIds),
    [filteredProjects],
  );
  ...
  useEffect(() => {
    ...
    const moveCursor = (event: MouseEvent) => {
      ...
      if (cursorRingRef.current)
        cursorRingRef.current.style.transform = position;
    };
  ...
  useEffect(() => {
    if (!selectedProject && !selectedExperience) return;
  ...
  const downloadCv = () => {
    window.open(
      `${import.meta.env.BASE_URL}cv/Amine_Boulila_CV.pdf`,
      "_blank",
      "noopener,noreferrer",
    );
  ...
  const placeholderLink = (label: string) =>
    showNotice(`${label} URL placeholder — add the public link when ready.`);
    ...
    <div className="noise min-h-[100dvh] overflow-x-hidden bg-background text-foreground">
      <header className="sticky top-0 z-40 border-b border-border/70 bg-background/85 backdrop-blur-xl">
        <div className="mx-auto flex h-[4.5rem] max-w-7xl items-center justify-between px-5 lg:px-10">
          ...
          >
            <span className="grid size-9 place-items-center rounded-md bg-primary text-primary-foreground transition-transform group-hover:rotate-[-6deg]">
              <Terminal size={17} />
            </span>
            <span className="font-mono text-sm font-medium tracking-tight">
              amine<span className="text-primary">.</span>boulila
            </span>
          </a>
          <nav
          ...
          >
            {navItems.map((item) => (
              <a
                key={item.href}
                href={item.href}
                className="font-mono text-[11px] uppercase tracking-[.13em] text-muted-foreground transition-colors hover:text-foreground"
          ...
          <div className="flex items-center gap-2">
            ...
            <a
              href="#contact"
              className="hidden items-center gap-2 rounded-md bg-primary px-3 py-2 font-mono text-[11px] font-medium uppercase tracking-[.1em] text-primary-foreground transition hover:-translate-y-0.5 hover:shadow-[0_8px_24px_hsl(var(--primary)/.18)] sm:flex"
              data-testid="link-header-contact"
            >
              Open to internship <ArrowUpRight size={14} />
            </a>
            <button
            ...
            <nav className="grid gap-1" aria-label="Mobile navigation">
              {navItems.map((item) => (
                <a
                  ...
                  href={item.href}
                  className="rounded-md px-3 py-3 font-mono text-xs uppercase tracking-[.12em] text-muted-foreground hover:bg-muted hover:text-foreground"
      ...
      <main id="top">
        <section className="relative isolate overflow-hidden border-b border-border/70">
          ...
          <div className="mx-auto grid max-w-7xl gap-14 px-5 pb-24 pt-16 md:grid-cols-[1.1fr_.9fr] md:items-end md:pb-32 md:pt-24 lg:px-10">
            <div className="reveal">
              ...
              <p className="mt-8 max-w-xl text-lg leading-8 text-muted-foreground md:text-xl">
                ...
                — a software engineering student building AI-powered
                applications, full-stack systems and cloud-ready software.
              </p>
              <div className="mt-8 flex flex-wrap items-center gap-3">
                <a
                  href="#projects"
                  className="group inline-flex items-center gap-2 rounded-md bg-primary px-5 py-3.5 text-sm font-semibold text-primary-foreground transition hover:-translate-y-1 hover:shadow-[0_12px_30px_hsl(var(--primary)/.2)]"
                ...
                <button
                  onClick={downloadCv}
                  className="inline-flex items-center gap-2 rounded-md border border-border bg-background/40 px-5 py-3.5 text-sm font-semibold transition hover:-translate-y-1 hover:border-primary hover:text-primary"
              ...
              <div className="mt-8 flex flex-wrap items-center gap-5 font-mono text-xs text-muted-foreground">
                <a
                  href="https://github.com/amine-boulila"
                  target="_blank"
                  rel="noreferrer"
                ...
                <a
                  href="https://www.linkedin.com/in/amin-boulila/"
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 transition hover:text-primary"
          ...
          <div className="mx-auto flex max-w-7xl items-center gap-5 px-5 pb-7 font-mono text-[10px] uppercase tracking-[.16em] text-muted-foreground lg:px-10">
            <span className="h-px w-10 bg-border" /> Scroll to inspect the
            workbench <ChevronDown size={13} />
          </div>
        </section>

        <section
          id="about"
          className="mx-auto max-w-7xl scroll-mt-20 px-5 py-24 lg:px-10 lg:py-32"
        >
          <SectionIntro
            number="01"
            label="About"
            title="Engineering with a reason to ship."
          />
          <div className="mt-14 grid gap-12 md:grid-cols-[.72fr_1.28fr] md:gap-24">
            <p className="line-accent max-w-sm text-2xl font-semibold leading-tight tracking-[-.03em]">
              The interesting part is where software meets the messy, useful
              world.
            </p>
            <div className="max-w-2xl space-y-6 text-base leading-8 text-muted-foreground">
              <p>
                I’m studying Software Engineering at the Faculty of Sciences of
                Tunis, with a focus on the systems behind useful products — the
                APIs, data flows, models and infrastructure that make an idea
                dependable.
              </p>
              <p>
                My work sits across AI/ML, full-stack engineering, cloud and
                DevOps, data engineering, and distributed systems. I like
                solving practical engineering problems and turning experiments
                into software that can be understood, extended and run.
              </p>
              <p className="text-foreground">
                I’m looking for a challenging{" "}
                <span className="text-primary">
                  4–6 month internship / PFE opportunity starting January 2027
                </span>
                , with the possibility of continuing full-time afterward.
              </p>
            </div>
          </div>
          <div className="mt-16 grid gap-px overflow-hidden rounded-xl border border-border bg-border sm:grid-cols-3">
            <InfoCell
              ...
              value="AI · full-stack · data · cloud"
            />
            <InfoCell
              icon={<Code2 size={18} />}
              label="Approach"
            ...
            <InfoCell
              ...
              label="Currently"
              value="Open to internship · 01.2027"
            />
          </div>
          <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-2 rounded-xl border border-border bg-card px-5 py-4">
            <span className="font-mono text-[10px] uppercase tracking-[.12em] text-primary">
              Languages
            </span>
        ...
        >
          <div className="mx-auto max-w-7xl px-5 py-24 lg:px-10 lg:py-32">
            <SectionIntro
              number="02"
              label="Tech stack"
              title="A wide toolkit, grounded in fundamentals."
              description="No percentages. No decorative skill bars. Just the technologies I use to think, build, connect, and ship."
            />
            <div className="mt-14 grid gap-px overflow-hidden rounded-xl border border-border bg-border md:grid-cols-2 lg:grid-cols-3">
              {skillGroups.map((group) => (
                <div
                  key={group.label}
                  className="group bg-background p-6 transition hover:bg-muted/60"
                >
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {group.items.map((item) => (
                      <span
                        key={item}
                        className="inline-flex items-center gap-1.5 rounded border border-border bg-card px-2.5 py-1.5 font-mono text-[11px] text-muted-foreground transition group-hover:border-primary/30"
                      >
        ...
        >
          <div className="flex flex-col justify-between gap-8 md:flex-row md:items-end">
            <SectionIntro
              ...
              description="A living record of systems, interfaces, and experiments — with the implementation details left visible."
            />
            <div className="flex flex-wrap gap-2 md:pb-1">
              {(
                ...
                <button
                  ...
                  className={`rounded-full border px-3.5 py-2 font-mono text-[11px] transition ${filter === item ? "border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground hover:border-primary hover:text-primary"}`}
          ...
          {filteredProjects.length === 0 ? (
            <div className="mt-14 rounded-xl border border-dashed border-border p-16 text-center">
                ...
                <div className="mb-6 flex items-end justify-between gap-4">
                  <div>
                    ...
                    <h3 className="mt-2 text-2xl font-semibold tracking-tight">
                      Core systems and products.
                    </h3>
                  </div>
                  <span className="hidden font-mono text-[10px] uppercase tracking-[.14em] text-muted-foreground sm:block">
                    01 — 03
                  </span>
                </div>
        ...
        >
          <SectionIntro
            number="05"
            label="Education"
            title="A foundation for the long problems."
          />
          <div className="mt-14 grid max-w-4xl gap-6">
            ...
            <div className="rounded-xl border border-primary/25 bg-primary/10 p-7 md:p-9">
              <div className="mb-12 flex items-center justify-between">
                <div className="grid size-11 place-items-center rounded-lg bg-primary/10 text-primary">
                  <GraduationCap size={21} />
                </div>
              ...
              <h3 className="text-2xl font-semibold tracking-tight">
                Pre-Engineering{" "}
                <span className="text-primary">(Top 10 of cohort)</span>
              </h3>
              <p className="mt-2 text-muted-foreground">
                Faculty of Science of Tunis
              </p>
        ...
        >
          <div className="mx-auto max-w-7xl px-5 py-24 lg:px-10 lg:py-32">
            <SectionIntro
              ...
              description="A snapshot of the things I’m actively building, learning, and contributing to."
            />
            <div className="mt-14 grid gap-px overflow-hidden rounded-xl border border-border bg-border md:grid-cols-2">
              {buildTracks.map((track, index) => (
                <div
                  key={track.title}
                  className="bg-background p-7 transition hover:bg-muted/60 md:p-9"
                >
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs text-muted-foreground">
                      0{index + 1}
                    </span>
                    <span className="inline-flex items-center gap-2 font-mono text-[10px] uppercase tracking-[.12em] text-primary">
                      <Circle size={8} fill="currentColor" /> {track.status}
                    </span>
                  </div>
                  <h3 className="mt-12 max-w-sm text-xl font-semibold tracking-tight">
                    {track.title}
                  </h3>
          ...
          <div className="mx-auto max-w-7xl px-5 py-24 lg:px-10 lg:py-32">
            <div className="grid gap-14 lg:grid-cols-[1fr_.85fr] lg:gap-24">
              <div>
                ...
                <h2 className="mt-5 max-w-xl text-5xl font-bold leading-[.98] tracking-[-.055em] sm:text-6xl">
                  Let’s build something useful.
                </h2>
                <p className="mt-7 max-w-lg text-lg leading-8 text-muted-foreground">
                  I’m currently looking for a 4–6 month internship / PFE
                  starting January 2027 in AI/ML, full-stack engineering,
                  cloud/DevOps or related software engineering roles.
                </p>
              ...
              >
                <div className="mb-8 flex items-center justify-between">
                  ...
                  <Send size={17} className="text-muted-foreground" />
                </div>
                <div className="grid gap-5">
                  ...
                  <p className="font-mono text-[10px] leading-5 text-muted-foreground">
                    Your message is sent securely through Formspree.
                  </p>
                </div>
              </form>
            </div>
      ...
      <footer className="border-t border-border">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-5 py-7 font-mono text-[10px] uppercase tracking-[.12em] text-muted-foreground sm:flex-row sm:items-center sm:justify-between lg:px-10">
          <span>© {new Date().getFullYear()} Amine Boulila</span>
          <span className="flex items-center gap-2">
            <Zap size={12} className="text-primary" /> Built with curiosity ·
            Tunisia
          </span>
          <a
            href="#top"
      ...
      {notice && (
        <div
          role="status"
          className="fixed bottom-5 left-1/2 z-50 flex w-[calc(100%-2rem)] max-w-md -translate-x-1/2 items-start gap-3 rounded-lg border border-primary/30 bg-card px-4 py-3 text-sm shadow-2xl"
        >
...
function SectionIntro({
  ...
  return (
    <div className="max-w-2xl">
      ...
      <h2 className="mt-5 text-4xl font-bold leading-[1.02] tracking-[-.05em] sm:text-5xl">
        {title}
      </h2>
      {description && (
...
function TechnologyIcon({ name }: { name: string }) {
  ...
  return (
    <Icon size={13} className="shrink-0 text-primary/80" aria-hidden="true" />
  );
}
...
function SystemMap({
  ...
  const activeLabel = systemMapNodes.find(
    (node) => node.id === activeNode,
  )?.label;
  const isConnected = (connection: string[]) =>
    activeNode === null || connection.includes(activeNode);

  return (
    <div
      className="system-map absolute inset-0"
    ...
    >
      <svg
        ...
        aria-label="Interactive map of Amine's technical focus areas"
      >
        <defs>
        ...
        {systemMapConnections.map(([fromId, toId]) => {
          ...
          return (
            <line
              ...
              x2={to.x}
              y2={to.y}
              className={`system-map-line ${highlighted ? "is-connected" : "is-muted"}`}
            />
          );
        })}
        {systemMapNodes.map((node, index) => {
          ...
          return (
            <g
              ...
              className={`system-map-node system-map-node-${index + 1} ${isActive ? "is-active" : ""}`}
            >
              <circle
                cx={node.x}
                cy={node.y}
                r={node.radius + 8}
                className="system-map-halo"
              />
              <circle
                cx={node.x}
                cy={node.y}
                ...
                filter={isActive ? "url(#system-map-glow)" : undefined}
              />
              <text
                x={node.x + 12}
                y={node.y - 10}
                className="system-map-label"
              >
                {node.label}
              </text>
            </g>
      ...
      <div className="float-slow absolute right-5 top-5 grid size-12 place-items-center rounded-lg border border-primary/30 bg-primary/10 text-primary">
        <Network size={20} />
      </div>
      <div
        className="absolute bottom-5 right-5 font-mono text-[9px] uppercase tracking-[.14em] text-muted-foreground/80"
        aria-live="polite"
      >
        {activeLabel ? `focus · ${activeLabel}` : "hover nodes"}
      </div>
    </div>
  );
}
...
function ProjectCard({
  ...
  const openOnKey = (event: React.KeyboardEvent<HTMLElement>) => {
  ...
  return (
    <article
      ...
      onClick={onOpen}
      onKeyDown={openOnKey}
      className={`group relative cursor-pointer overflow-hidden rounded-xl border bg-card transition duration-300 hover:-translate-y-1 hover:border-primary/50 hover:shadow-[0_18px_50px_hsl(var(--background)/.25)] ${project.standout ? "border-primary/30 lg:col-span-2" : featured ? "border-primary/15" : "border-border"}`}
    ...
    >
      ...
      <div className="flex min-h-64 flex-col justify-between p-6 md:min-h-72 md:p-8">
        <div>
          ...
          <h3 className="max-w-3xl text-2xl font-semibold tracking-[-.03em] md:text-3xl">
            {project.name}
          </h3>
        ...
        <div className="mt-10 flex items-center gap-2 border-t border-border pt-5 font-mono text-[10px] uppercase tracking-[.12em] text-primary">
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="relative max-h-[min(780px,calc(100dvh-2rem))] w-full max-w-3xl overflow-y-auto rounded-xl border border-primary/30 bg-card p-6 shadow-2xl sm:p-9"
      >
        <button
          onClick={onClose}
          className="absolute right-5 top-5 grid size-9 place-items-center rounded-md border border-border text-muted-foreground transition hover:border-primary hover:text-primary"
        ...
        <div className="pr-12">
          ...
          <h2
            ...
            className="mt-4 max-w-2xl text-3xl font-bold tracking-[-.04em] sm:text-4xl"
          >
            {project.name}
          </h2>
        ...
        {project.architecture && (
          <div className="mt-8 rounded-lg border border-primary/20 bg-primary/5 p-4 font-mono text-xs leading-6 text-primary sm:text-sm">
            {project.architecture}
          </div>
        )}
        <div className="mt-8 grid gap-6 border-t border-border pt-7 sm:grid-cols-2">
          ...
          <div>
            ...
            <p className="mt-3 text-sm leading-7">{project.contribution}</p>
          </div>
        </div>
        <div className="mt-8 border-t border-border pt-7">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {project.technologies.map((technology) => (
              <span
                key={technology}
                className="rounded border border-border bg-background px-2.5 py-1.5 font-mono text-[11px] text-muted-foreground"
              >
                {technology}
              </span>
            ))}
        ...
        <div className="mt-8 flex flex-wrap gap-3 border-t border-border pt-7">
          <button
            onClick={() =>
              onNotice(
                `${project.name}: GitHub URL placeholder — add the repository link when ready.`,
              )
            }
            className="inline-flex items-center gap-2 rounded-md border border-border px-4 py-3 font-mono text-xs uppercase tracking-[.1em] transition hover:border-primary hover:text-primary"
            data-testid={`button-modal-github-${project.id}`}
          >
            <Github size={15} /> Code placeholder
          </button>
          <button
            onClick={() =>
              onNotice(
                `${project.name}: project demo placeholder — add a demo link when ready.`,
              )
            }
            className="inline-flex items-center gap-2 rounded-md border border-border px-4 py-3 font-mono text-xs uppercase tracking-[.1em] transition hover:border-primary hover:text-primary"
            data-testid={`button-modal-demo-${project.id}`}
          >
            <ExternalLink size={15} /> Demo placeholder
          </button>
        </div>
      </div>
    </div>
...
function ExperienceCard({
  ...
  return (
    <article
      ...
      onClick={onOpen}
      onKeyDown={openOnKey}
      className="group grid cursor-pointer gap-5 border-t border-border py-8 transition hover:border-primary/50 md:grid-cols-[7rem_1fr] md:gap-10"
      data-testid={`card-experience-${index + 1}`}
      aria-label={`Open details for ${experience.company}`}
    >
      <div className="font-mono text-sm text-primary">{experience.period}</div>
      <div>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {experience.focus.map((item) => (
            <span
              key={item}
              className="rounded border border-border bg-background px-2.5 py-1.5 font-mono text-[11px] text-muted-foreground transition group-hover:border-primary/40"
            >
              {item}
            </span>
          ))}
