...
function Home() {
  ...
  useEffect(() => {
    ...
    const moveCursor = (event: MouseEvent) => {
      ...
      if (cursorRingRef.current) cursorRingRef.current.style.transform = position;
    };
  ...
  return (
    <div className="noise min-h-[100dvh] overflow-x-hidden bg-background text-foreground">
          ...
          <div className="mx-auto grid max-w-7xl gap-14 px-5 pb-24 pt-16 md:grid-cols-[1.1fr_.9fr] md:items-end md:pb-32 md:pt-24 lg:px-10">
            <div className="reveal">
              ...
              <div className="mt-8 flex flex-wrap items-center gap-3">
                ...
                <a href="https://www.linkedin.com/in/amin-boulila/" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 transition hover:text-primary" data-testid="button-hero-linkedin"><Linkedin size={15} /> LinkedIn</a>
      ...
      <footer className="border-t border-border"><div className="mx-auto flex max-w-7xl flex-col gap-4 px-5 py-7 font-mono text-[10px] uppercase tracking-[.12em] text-muted-foreground sm:flex-row sm:items-center sm:justify-between lg:px-10"><span>© {new Date().getFullYear()} Amine Boulila</span><span className="flex items-center gap-2"><Zap size={12} className="text-primary" /> Built with curiosity · Tunisia</span><a href="#top" className="transition hover:text-primary" data-testid="link-back-top">Back to top ↑</a></div></footer>
       ...
       {selectedProject && <ProjectModal project={selectedProject} onClose={() => setSelectedProject(null)} onNotice={showNotice} />}
    </div>
  );
}
...
function ProjectCard({ project, index, onNotice, onOpen }: { project: (typeof projects)[number]; index: number; onNotice: (message: string) => void; onOpen: () => void }) {
  ...
  return <article role="button" tabIndex={0} onClick={onOpen} onKeyDown={openOnKey} className={`group relative cursor-pointer overflow-hidden rounded-xl border bg-card transition duration-300 hover:-translate-y-1 hover:border-primary/50 hover:shadow-[0_18px_50px_hsl(var(--background)/.25)] ${project.standout ? 'border-primary/30 lg:col-span-2' : 'border-border'}`} data-testid={`card-project-${project.id}`} aria-label={`Open details for ${project.name}`}>
    <div className="absolute right-6 top-6 font-mono text-[10px] text-muted-foreground">0{index + 1} / {project.category}</div>
    <div className="p-6 md:p-8"><div className="mb-12 flex items-center gap-3"><span className="grid size-10 place-items-center rounded-md bg-primary/10 text-primary">{project.category === 'AI / ML' ? <Sparkles size={18} /> : project.category === 'Full-stack' ? <Layers3 size={18} /> : <Server size={18} />}</span><span className="font-mono text-[10px] uppercase tracking-[.15em] text-primary">{project.eyebrow}</span></div><div className="flex items-start justify-between gap-5"><h3 className="max-w-3xl text-2xl font-semibold tracking-[-.03em] md:text-3xl">{project.name}</h3><span className="hidden shrink-0 font-mono text-[10px] uppercase tracking-[.12em] text-primary sm:inline-flex sm:items-center sm:gap-1">Inspect <ArrowUpRight size={13} /></span></div><p className="mt-4 max-w-2xl leading-7 text-muted-foreground">{project.description}</p>{project.architecture && <div className="mt-7 rounded-lg border border-primary/20 bg-primary/5 p-4 font-mono text-xs leading-6 text-primary md:text-sm">{project.architecture}</div>}<div className={`mt-8 grid gap-7 border-t border-border pt-7 ${project.standout ? 'md:grid-cols-2' : ''}`}><div><p className="font-mono text-[10px] uppercase tracking-[.14em] text-muted-foreground">The problem</p><p className="mt-3 text-sm leading-6">{project.problem}</p></div><div><p className="font-mono text-[10px] uppercase tracking-[.14em] text-muted-foreground">My contribution</p><p className="mt-3 text-sm leading-6">{project.contribution}</p></div></div><div className="mt-8 flex flex-wrap items-center justify-between gap-4"><div className="flex flex-wrap gap-2">{project.technologies.map((technology) => <span key={technology} className="rounded border border-border px-2 py-1 font-mono text-[10px] text-muted-foreground">{technology}</span>)}</div><div className="flex shrink-0 gap-3"><button onClick={(event) => { event.stopPropagation(); onNotice(`${project.name}: GitHub URL placeholder — add the repository link when ready.`); }} className="inline-flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-[.1em] text-muted-foreground transition hover:text-primary" data-testid={`button-project-github-${project.id}`}><Github size={14} /> Code</button><button onClick={(event) => { event.stopPropagation(); onNotice(`${project.name}: project demo placeholder — add a demo link when ready.`); }} className="inline-flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-[.1em] text-muted-foreground transition hover:text-primary" data-testid={`button-project-demo-${project.id}`}><ExternalLink size={14} /> Demo</button></div></div></div>
  </article>;
}

function ProjectModal({ project, onClose, onNotice }: { project: (typeof projects)[number]; onClose: () => void; onNotice: (message: string) => void }) {
  return <div className="fixed inset-0 z-[60] grid place-items-center p-4 sm:p-6" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="absolute inset-0 bg-background/80 backdrop-blur-md" />
    <div role="dialog" aria-modal="true" aria-labelledby="project-modal-title" className="relative max-h-[min(780px,calc(100dvh-2rem))] w-full max-w-3xl overflow-y-auto rounded-xl border border-primary/30 bg-card p-6 shadow-2xl sm:p-9">
      <button onClick={onClose} className="absolute right-5 top-5 grid size-9 place-items-center rounded-md border border-border text-muted-foreground transition hover:border-primary hover:text-primary" aria-label="Close project details" data-testid="button-close-project-modal"><X size={17} /></button>
      <div className="pr-12"><p className="font-mono text-[10px] uppercase tracking-[.16em] text-primary">{project.category} / {project.eyebrow}</p><h2 id="project-modal-title" className="mt-4 max-w-2xl text-3xl font-bold tracking-[-.04em] sm:text-4xl">{project.name}</h2><p className="mt-5 max-w-2xl text-base leading-7 text-muted-foreground">{project.description}</p></div>
      {project.architecture && <div className="mt-8 rounded-lg border border-primary/20 bg-primary/5 p-4 font-mono text-xs leading-6 text-primary sm:text-sm">{project.architecture}</div>}
      <div className="mt-8 grid gap-6 border-t border-border pt-7 sm:grid-cols-2"><div><p className="font-mono text-[10px] uppercase tracking-[.14em] text-muted-foreground">The problem</p><p className="mt-3 text-sm leading-7">{project.problem}</p></div><div><p className="font-mono text-[10px] uppercase tracking-[.14em] text-muted-foreground">My contribution</p><p className="mt-3 text-sm leading-7">{project.contribution}</p></div></div>
      <div className="mt-8 border-t border-border pt-7"><p className="font-mono text-[10px] uppercase tracking-[.14em] text-muted-foreground">Technologies</p><div className="mt-4 flex flex-wrap gap-2">{project.technologies.map((technology) => <span key={technology} className="rounded border border-border bg-background px-2.5 py-1.5 font-mono text-[11px] text-muted-foreground">{technology}</span>)}</div></div>
      <div className="mt-8 flex flex-wrap gap-3 border-t border-border pt-7"><button onClick={() => onNotice(`${project.name}: GitHub URL placeholder — add the repository link when ready.`)} className="inline-flex items-center gap-2 rounded-md border border-border px-4 py-3 font-mono text-xs uppercase tracking-[.1em] transition hover:border-primary hover:text-primary" data-testid={`button-modal-github-${project.id}`}><Github size={15} /> Code placeholder</button><button onClick={() => onNotice(`${project.name}: project demo placeholder — add a demo link when ready.`)} className="inline-flex items-center gap-2 rounded-md border border-border px-4 py-3 font-mono text-xs uppercase tracking-[.1em] transition hover:border-primary hover:text-primary" data-testid={`button-modal-demo-${project.id}`}><ExternalLink size={15} /> Demo placeholder</button></div>
    </div>
  </div>;
}
